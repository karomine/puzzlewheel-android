...
public abstract class GridTile {
    ...
    //creating path over and over maybe later store paths at creation and change when change size
    public void drawFill() {
        for (int i = 0; i < colorScheme.size(); i++) {
            Path path = TileSectionPaths.createPath(i, this.xOffset, this.yOffset, this.scale);
            this.backgroundCanvas.drawPath(path, this.colorScheme.getPaint(i));
        }
    }
    ...
    public void drawBoundaries() {
        ...
        for (int i = 0; i < 51; i++) {
            ...
            if (comp.comparePaints(p1, p2)) {
                this.drawBoundary(i, PaintColors.BLACK_PAINT, 2);
            } else {
    ...
    public boolean matchInRanges(int[][] ranges, GridTile other) {
        ...
        for (int[] range : ranges) {
            for (int section : range) {
                ...
                p2 = colorScheme.getPaint(section);
    ...
    public BasePiece topPiece(int xOffset, int yOffset) {
